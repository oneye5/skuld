...
@pytest.fixture
def test_spec():
    """Load the momentum baseline spec for testing."""
    python_root = Path(__file__).parent.parent
    spec_path = python_root / "configs" / "strategy-specs" / "archive" / "mom-baseline.yaml"
    return load_spec(spec_path)


def test_write_recommendations_csv_creates_files(
    tmp_path: Path,
    minimal_trade_list: TradeList,
    test_spec,
):
...
def test_write_recommendations_csv_required_columns(
    tmp_path: Path,
    minimal_trade_list: TradeList,
    test_spec,
):
...
def test_write_recommendations_csv_factor_columns_match_spec(
    tmp_path: Path,
    minimal_trade_list: TradeList,
    test_spec,
):
    ...
    # mom-baseline has only momentum
    assert "factor_momentum_z" in df.columns
