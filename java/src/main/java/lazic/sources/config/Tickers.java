package lazic.sources.config;

public class Tickers {

	public static final String[] _TICKERS = { // small subset
					"ANZ.NZ", "AFCA.NZ", "AFI.NZ",   "AFT.NZ", "AGG.NZ"
	};

	public static final String[] TICKERS = {
					"ANZ.NZ", "AFCA.NZ", "AFI.NZ",   "AFT.NZ", "AGG.NZ", "AGL.NZ", "AIA.NZ", "AIR.NZ", "ALF.NZ", "2CC2.NZ",
					"AOF.NZ", "APA.NZ", "APL.NZ", "ARB.NZ", "ARG.NZ", "ASD.NZ", "ASF.NZ", "ASP.NZ", "ASR.NZ", "ATM.NZ",
					"AUE.NZ", "AUS.NZ", "BAI.NZ", "BFG.NZ", "BGP.NZ", "BIF.NZ", "BIT.NZ", "BLT.NZ", "BOT.NZ", "BPG.NZ",
					"BRM.NZ", "BRW.NZ", "BTC.NZ", "CCC.NZ", "CDI.NZ", "CEN.NZ", "CHI.NZ", "CMO.NZ", "CNU.NZ",
					"CRP.NZ", "CVT.NZ", "DGL.NZ", "DIV.NZ", "DOW.NZ", "EBO.NZ", "EMF.NZ", "EMG.NZ", "ENS.NZ", "ERD.NZ",
					"ESG.NZ", "EUF.NZ", "EUG.NZ", "FBU.NZ", "FCG.NZ", "FCT.NZ", "FNZ.NZ", "FPH.NZ", "FRW.NZ", "FSF.NZ",
					"FWL.NZ", "GBF.NZ", "GEN.NZ", "GGB.NZ", "GLD.NZ", "GMT.NZ", "GNE.NZ", "GPR.NZ", "GTK.NZ", "GXH.NZ",
					"HFL.NZ", "HGH.NZ", "HLG.NZ", "IFT.NZ", "IKE.NZ", "INF.NZ", "IPL.NZ", "IPR.NZ", "JPN.NZ", "KFL.NZ",
					"KFLW.NZ", "KMD.NZ", "KPG.NZ","LIC.NZ", "LIV.NZ", "MCK.NZ", "MCYP.NZ", "MCY.NZ", "MDZ.NZ", "MEE.NZ",
					"MEL.NZ", "MFB.NZ", "MFT.NZ", "MHJ.NZ", "MLN.NZ", "MMH.NZ", "MNW.NZ", "MOV.NZ", "MPG.NZ", "MZY.NZ",
					"NGB.NZ", "NPF.NZ", "NPH.NZ", "NTL.NZ", "NWF.NZ", "NZB.NZ", "NZC.NZ", "NZG.NZ", "NZK.NZ", "NZL.NZ",
					"NZLW.NZ", "NZM.NZ", "NZTS.NZ", "NZX.NZ", "OCA.NZ", "OZY.NZ", "PCT.NZ", "PEB.NZ", "PFI.NZ", "PGW.NZ",
					"PHL.NZ", "PHLW.NZ", "PLP.NZ", "POT.NZ", "PYS.NZ", "RAD.NZ", "RAK.NZ", "RBD.NZ", "RTO.NZ", "RUA.NZ",
					"RYM.NZ", "SAN.NZ", "SCL.NZ", "SCT.NZ", "SDL.NZ", "SEK.NZ", "SKC.NZ", "SKL.NZ", "SKO.NZ", "SKT.NZ",
					"SMI.NZ", "SML.NZ", "SPG.NZ", "SPK.NZ", "SPN.NZ", "SPY.NZ", "STU.NZ", "SUM.NZ", "SVR.NZ", "TAH.NZ",
          // NON NZX TICKERS, FOR PROVIDING INFORMATION REGARDING OTHER MARKETS
          "%5EFTSE", // UK INDEX
          "ZS=F", // SOYBEANS
          "%5ETNX"
	};
}
