...
def plan_trades(
    ...
    target_shares = (target_values_full / target_prices).fillna(0).apply(lambda x: int(x) if x > 0 else 0)
    ...
    # 4. Compute weights
    current_weights = current_values_full / total_nav if total_nav > 1e-6 else pd.Series(0.0, index=all_tickers)
    target_weights = target_values_full / total_nav if total_nav > 1e-6 else pd.Series(0.0, index=all_tickers)
    ...
    # 8. Sharesies fee-cliff deferral
    # Rank tradeable positions by expected alpha
    tradeable = action.isin(["BUY", "SELL"])
    ...
    if expected_alpha is None:
        ...
        alpha_proxy = expected_alpha.reindex(all_tickers, fill_value=0.0).abs()
    
    # Order candidates by alpha descending
    candidates = all_tickers
    ranked = sorted(candidates, key=lambda t: alpha_proxy[t], reverse=True)
    
    cumulative_volume = 0.0
    deferred = pd.Series(False, index=all_tickers)
    
    for ticker in ranked:
        if not tradeable[ticker]:
            continue
        trade_vol = delta_values_abs[ticker]
        new_cumulative = cumulative_volume + trade_vol
        
        if new_cumulative > sharesies_coverage_nzd:
            # Trade would push us past coverage cap
            # Defer unless expected alpha > marginal cost
            marginal_cost_bps = sharesies_excess_bps
            ticker_alpha_bps = (
                float(expected_alpha.get(ticker, 0.0))
                if expected_alpha is not None
                else 0.0
            )
            
            if ticker_alpha_bps < marginal_cost_bps:
                action[ticker] = "DEFER"
    ...
    # Total cost = spread + Sharesies fee
    total_spread = executed["est_round_trip_cost_nzd"].sum() if not executed.empty else 0.0
    
    # Sharesies fee (using cost_model logic)
    exec_values = executed["delta_value_nzd"].abs() if not executed.empty else pd.Series(dtype=float)
