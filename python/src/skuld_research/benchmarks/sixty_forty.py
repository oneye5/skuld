...
def sixty_forty(
    ...
    Args:
        ...
        bond_macro_field: macro field name for bond rates (default "long_term_interest_rates").
    ...
    # Auto-detect if rates are in percentage (> 1.0 means likely percentage)
    if not bond_rates_monthly.empty and bond_rates_monthly.mean() > 1.0:
        bond_rates_monthly = bond_rates_monthly / 100.0
        notes_list.append("Bond rates auto-converted from percentage to decimal")
    
    # Monthly bond return: (1 + r_annual)^(1/12) - 1
