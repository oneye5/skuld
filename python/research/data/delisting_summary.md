...
1. **Sample size.** 13 loss-type delistings is small. The bootstrap CI is tight because of the concentrated loss distribution (most are −100%), but the *rate estimate* is sensitive to whether a few borderline cases are included or excluded.

2. **Selection bias.** This dataset captures delistings from public records. Companies that quietly faded from the NZX without formal announcements may be underrepresented. The Wikipedia/delisted.co.nz sources are reasonably comprehensive but not guaranteed complete.

3. **Terminal return approximation.** Terminal returns are estimated from publicly reported prices at key dates (suspension, IPO, peaks). Exact last-traded prices 12 months prior to delisting are approximated from news reporting, not tick-level data. For the 8 total-loss cases, the terminal return of −100% is exact. For the others, the 12-month-prior prices carry ±10–20% estimation error.

4. **Factor quintile assessment.** Factor quintiles are assessed qualitatively from company characteristics at delisting, not computed from the actual factor model (which doesn't exist yet). These will be refined in Milestone 5 when the factor model is operational. The qualitative assessments are directionally reliable: a company trading at $0.03 with negative earnings is unambiguously in value Q5 and momentum Q1.

5. **Survivorship of the universe itself.** This study only captures delistings from the NZX. It does not capture stocks that never listed in the first place, or companies that declined so far they fell below inclusion thresholds without formally delisting. The "living dead" bias is not addressed here.

6. **Period specificity.** The 2000–2025 period includes the GFC (2008–09), the NZ earthquake sequence (2010–11), and COVID-19 (2020). Delisting rates may be higher in crisis periods and lower in benign periods. The sample is too small to estimate time-varying rates reliably.
